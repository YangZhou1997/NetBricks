...
fn macswap(packet: RawPacket) -> Result<Ipv4> {
    ...
    let decrypted_pkt_len = aes_gcm128_decrypt(payload, decrypted_pkt, false).unwrap();
    ...
    let encrypted_pkt_len = aes_gcm128_encrypt(&decrypted_pkt[..(decrypted_pkt_len - ESP_HEADER_LENGTH - AES_CBC_IV_LENGTH)], &(*esp_hdr), payload).unwrap();
